...
public class ecc {
    ...
    public static BaggingML EccTest( String file, int clusterNum) throws Exception {
        // 8 clusters
//        for (int i = 0; i < 8; i++) {
...
//        h.setClassifier(cc);
        ...
        Cluster_CC_Builder cluster_cc_builder= new Cluster_CC_Builder(clusterNum,dataset,0.0);
        ...
        PrintWriter writer = new PrintWriter("result"+ clusterNum + ".csv", "UTF-8");
...
//        – a random subset of D
        ...
        for (int i=0;i<8;i++){
            ecc.EccTest("src/main/CAL500_clustered_adjusted.arff", i);
        }
    }
}
